...
//<<< DDD / CQRS
    ...
    public void when_then_UPDATE_(@Payload  ) {
        try {
            if (!.validate()) return;
                ...
                List<MySalesList> mySalesListList = mySalesListRepository.findByPostId(.getPostId());
                for(MySalesList mySalesList : mySalesListList){
                    // view 객체에 이벤트의 eventDirectValue 를 set 함
                    mySalesList.setStatus(.getStatus());
                    mySalesList.set();
                // view 레파지 토리에 save
                mySalesListRepository.save(mySalesList);
                }
    ...
    public void whenPostEdited_then_UPDATE_2(@Payload PostEdited postEdited) {
        try {
            if (!postEdited.validate()) return;
                ...
                List<MySalesList> mySalesListList = mySalesListRepository.findByPostId(postEdited.getPostId());
                for(MySalesList mySalesList : mySalesListList){
                    // view 객체에 이벤트의 eventDirectValue 를 set 함
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenDealReserved_then_UPDATE_3(@Payload DealReserved dealReserved) {
        try {
            if (!dealReserved.validate()) return;
                // view 객체 조회

                List<MySalesList> mySalesListList = mySalesListRepository.findByPostId(dealReserved.getPostId());
                for(MySalesList mySalesList : mySalesListList){
                    // view 객체에 이벤트의 eventDirectValue 를 set 함
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenDealCanceled_then_UPDATE_4(@Payload DealCanceled dealCanceled) {
        try {
            if (!dealCanceled.validate()) return;
                // view 객체 조회

                List<MySalesList> mySalesListList = mySalesListRepository.findByPostId(dealCanceled.getPostId());
                for(MySalesList mySalesList : mySalesListList){
                    // view 객체에 이벤트의 eventDirectValue 를 set 함
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenDealEnded_then_UPDATE_5(@Payload DealEnded dealEnded) {
        try {
            if (!dealEnded.validate()) return;
                // view 객체 조회

                List<MySalesList> mySalesListList = mySalesListRepository.findByPostId(dealEnded.getPostId());
                for(MySalesList mySalesList : mySalesListList){
                    // view 객체에 이벤트의 eventDirectValue 를 set 함
